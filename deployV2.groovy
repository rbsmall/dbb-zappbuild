...
/** NJL - bug in code.  when a link card exsists, it is output twice as a load mod buztool fails
 *  fix ...?
 */
 ...
 *
 **NJL added version as an option
 *  -v,--version <ver>            ver for the component
 *
...
println("**deploy.groovy Started  v1.1:  For component and version: $properties.component  $properties.version" )
...
def reportInpFile = properties.workDir + "/BuildReport.json"
reportInpFile = reportInpFile.replaceAll("[\\n\\t ]", "");
println "reportInpFile - ${reportInpFile}"

// read build report data
println("** Read build report data from ${reportInpFile}")

// note: the current build.groovy creates stores logs in dir that are
// timestamped.  This practice is questionable..  are we keeping build logs on uss for archival?
// I would need to change the code below to find the latest BuildReport.json.  Instead
// For now I changed build.groovy to output to a static location
// A better long term soultion is to use the buildId from the pipeline tool like Azure.
// workdir point to /logs/build
//def jsonOutputFile = new File("${properties.workDir}/BuildReport.json")
def jsonOutputFile = new File("${reportInpFile}")
...
def cmd = [ properties.buztoolPath, "createzosversion",
	"-c", properties.component,
	"-s", "$properties.workDir/shiplist.xml",
	"-o", "${properties.workDir}/buztool.output "]

if (properties.uver)
	cmd.addAll(["-v", properties.uver])

// use verbose to debug but it sometimes causes the script to hang??
//	"-verb true", 
...
def parseInput(String[] cliArgs){
    ...
    cli.v(longOpt:'version', args:1, argName:'uver', 'Version of the UCD component')
...
//njl- needs to follow new zAppBuild structure ...  not used for the Az PoC
	...
	if (opts.v) properties.uver             = opts.v
...
//        assert properties.uver: "Missing UCD component version"
